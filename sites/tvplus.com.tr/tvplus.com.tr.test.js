...
// url() testi için gerçek site_id
const channelForUrl = {
  lang: 'tr',
  site_id: 'nick-jr/4353',
  xmltv_id: 'NickJr.tr'
}
// parser() testleri için — EPG API çağrılmaması için rakamsız site_id
const channel = {
  lang: 'tr',
  site_id: 'nick-jr-noId',
  xmltv_id: 'NickJr.tr'
}
...
it('can generate valid url', async () => {
  expect(await url({ channel: channelForUrl })).toBe(
...
it('can parse response', async () => {
  ...
  const results = (await parser({ date, channel, content })).map(p => {
